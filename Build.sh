...
if [ $_DOC -eq 1 ] ; then
{
   if [ -f ${0%/*}/tools/Build_Doc.sh ] ; then
   {
      ${0%/*}/tools/Build_Doc.sh
      if [ $? -ne 0 ] ; then
         echo "Build_Doc.sh failed !"
         end $_PAUSE 1
      fi
   }
   fi
...
if [ $_UTEST -eq 1 ] ; then
{
   if [ -f ${0%/*}/tools/Build_UTest.sh ] ; then
   {
      ${0%/*}/tools/Build_UTest.sh
      if [ $? -ne 0 ] ; then
         echo "Build_UTest.sh failed !"
         end $_PAUSE 1
      fi
   }
   fi
